# Profil Mahasiswa

**NAMA:** Linda Permata Sari
**NIM:** 20230140086

Pertemuan 1
![Foto Profil](public/images/pertemuan1.png)

Pertemuan 2

Halaman Login
![alt text](public/images/login.png)

Halaman Register
![alt text](public/images/register.png)

Halaman About
![alt text](public/images/profile.png)

Pertemuan 3

ERD
![alt text](public/images/erdprak.png)

Database Users
![alt text](public/images/dbusers.png)

Database Kategori
![alt text](public/images/dbkategori.png)

Database Produk
![alt text](public/images/dbproduk.png)

Pertemuan 4

Halaman Tambah Produk
![alt text](public/images/tambahproduk.png)

Halaman Edit Produk
![alt text](public/images/editproduk.png)

Halaman Hapus Produk
![alt text](public/images/hapusproduk.png)

Halaman Lihat Produk
![alt text](public/images/viewproduct.png)

Pertemuan 5

Halaman Produk Admin
![alt text](public/images/produkadmin.png)

Halaman Produk User
![alt text](public/images/produkuser.png) 

Pertemuan 6

Halaman Edit Produk (Menambahkan Validasi)
![alt text](public/images/validasieditproduk.png) 

Pertemuan 7

Tambah Button
![alt text](public/images/tambahbutton.png) 

UCP1 PWF

Halaman Kategori
![alt text](public/images/crudkategori.png) 

Halaman Form Add Kategori
![alt text](public/images/formkategori.png)

Form Add Product dengan Relasi Kategori
![alt text](public/images/formrelasikategori.png)

Gates untuk admin dan user
![alt text](public/images/gatesadmin.png)

![alt text](public/images/gatesuser.png)

Pertemuan 9

User GET API
![alt text](public/images/getapi.png)

POST Kategori API
![alt text](public/images/postkategoriapi.png)

GET Kategori API
![alt text](public/images/getkategori.png)

GET Kategori by id
![alt text](public/images/getkategoribyid.png)

PUT Kategori API
![alt text](public/images/updatekategori.png)

DELETE Kategori
![alt text](public/images/deletekategori.png)

GET Produk
![alt text](public/images/getproduk.png)

POST Produk
![alt text](public/images/postproduk.png)

GET Produk by id
![alt text](public/images/getprodukbyid.png)

PUT Produk
![alt text](public/images/updateproduk.png)

DELETE Produk
![alt text](public/images/deleteproduk.png)

